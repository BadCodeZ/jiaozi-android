...
/**
 * 教材库：导入 PDF / TXT / MD 教材原文，录入学段·教材·单元·课（可空）后结构化索引，
 * 可随备课信封（books 集合）跨端同步。全文存于 BookEntity.text。
...
fun BookScreen(nav: NavHostController) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            StatTile("教材条目", "$total", Modifier.weight(1f))
            StatTile("收录字数", "$chars", Modifier.weight(1f))
        }

        SectionTitle("教材原文")
        if (items.isEmpty()) {
            EmptyHint("book", "还没有教材", "点右上「导入」加入一份教材原文，可随备课信封跨端同步。")
        }
        LazyColumn(Modifier.fillMaxWidth().weight(1f).navigationBarsPadding(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            items(items, key = { it.id }) { e ->
                val head = listOf(e.book, e.unit, e.lesson).map { it.ifBlank { null } }.filterNotNull().joinToString(" · ")
                ItemCard(
                    icon = appPainter("book"),
                    title = head.ifBlank { "(未命名教材)" },
                    subtitle = e.grade.ifBlank { "未分类学段" },
                    onClick = { detail = e }
                ) {
                    IconButton(onClick = { scope.launch { appVm.repo.deleteBook(e.id) } }) {
    ...
    if (detail != null) {
        AlertDialog(
            ...
            title = { Text(detail!!.book.ifBlank { "(未命名教材)" }) },
            text = {
                Column(Modifier.fillMaxWidth().heightIn(max = 440.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    val head = listOf(detail!!.grade, detail!!.unit, detail!!.lesson).map { it.ifBlank { null } }.filterNotNull().joinToString(" · ")
                    Text(head, style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.primary)
                    ...
                    if (book.isNotBlank()) {
                        ...
                        scope.launch {
                            appVm.repo.upsertBook(
                                BookEntity(id = "B$now", grade = grade, book = book.trim(), unit = unit.trim(), lesson = lesson.trim(), text = importText ?: "", _mt = now)
                            )
                        }
                        importText = null
                    }
